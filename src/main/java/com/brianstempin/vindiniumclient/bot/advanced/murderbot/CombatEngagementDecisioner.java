...
public class CombatEngagementDecisioner implements Decision<AdvancedMurderBot.GameContext, BotMove> {
    ...
    public BotMove makeDecision(AdvancedMurderBot.GameContext context) {
        ...
        // If the difference between minegold and the opponents gold tehn I am going in defence mode 
        for(GameState.Position hpos: heroesByPosition.keySet())
        {
        	opponentgold=opponentgold+heroesByPosition.get(hpos).getGold();
        }
        
        if(MineGold-opponentgold>=50)
        {
