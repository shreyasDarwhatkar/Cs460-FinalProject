...
public class BotTargetingDecisioner implements Decision<AdvancedMurderBot.GameContext, BotMove> {
    ...
    public BotMove makeDecision(AdvancedMurderBot.GameContext context) {
        ...
        PriorityQueue<GameState.Hero> enemyQueue = new PriorityQueue<GameState.Hero>(100, new Comparator<GameState.Hero>() {
	        public int compare(GameState.Hero hero1,GameState.Hero hero2) {
	            return (hero1.getLife()>= hero2.getLife()) ? (1): (-1);
	        }
	    });
        ...
        Set<GameState.Hero> heroesWithlessHealth = new HashSet<>();
        ...
        //only attack players with less health than me
        for(Mine currentMine : context.getGameState().getMines().values()) {
            if(currentMine.getOwner() != null){
            	if(me.getLife()>currentMine.getOwner().getLife()){
                heroesWithlessHealth.add(currentMine.getOwner());
                enemyQueue.add(currentMine.getOwner());
                }
            }
        }
        ...
        AdvancedMurderBot.DijkstraResult closestTargetDijkstraResult = null;
            while (!enemyQueue.isEmpty()) {
            	GameState.Hero currentHero=enemyQueue.remove();
				...
				if (currentHero.getId() == context.getGameState().getMe()
						.getId())
					continue;
				//shreyas change wont spare anyone
				...
				Vertex currentHeroVertext = context.getGameState()
						.getBoardGraph().get(currentHero.getPos());
				for (Vertex currentVertext : currentHeroVertext
						.getAdjacentVertices()) {
					if (context.getGameState().getPubs()
							.containsKey(currentVertext.getPosition())) {
						continue;
					}
				}
				...
				if (closestTarget == null) {
					...
					closestTargetDijkstraResult = context
							.getDijkstraResultMap().get(closestTarget.getPos());
					continue;
				} else if (closestTargetDijkstraResult.getDistance() > currentDijkstraResult
						.getDistance()) {
					closestTarget = currentHero;
					closestTargetDijkstraResult = context
							.getDijkstraResultMap().get(closestTarget.getPos());
